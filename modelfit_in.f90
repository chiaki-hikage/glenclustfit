MODULE Readmodel
  implicit none
  character(len=6), parameter :: measurement='angcor'
  real :: rmin=0.1, rmax=2
  integer, parameter :: nlrg=3
  character(len=10), parameter :: clrg(3)=(/'highpcen','rmcg','blrg'/)
  character(len=100), parameter :: infname='data/redmapper.angcor.phys.new.'
  character(len=100), parameter :: inf2h='data/2halo_angcor.dat'
  character(len=1), parameter :: ccov='y',genNFW='n'
  character(len=5), parameter :: offmodel='gauss'
  double precision, parameter :: pi=3.1415926535897932384
  double precision, parameter :: z=0.25,omegam=0.3,gconst=4.3e-9,rhoc=3e4/(8*pi*gconst),rhom=rhoc*omegam*(1+z)**3
